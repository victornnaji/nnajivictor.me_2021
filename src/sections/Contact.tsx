...
const Contact = () => {
  ...
  const { email, footer_social } = data.contact.nodes[0].siteMetadata;
    ...
    <>
      <StyledContact id="contact">
        <div className="contact-column">
          <h3 className="contact-colum__title">
            <span className="line__inner-mask move-left--1">
              {SplitWord("Want to", "export-line__inner")}
            </span>
            <span className="line__inner-mask">
              {SplitWord("Start", "export-line__inner")}
            </span>
            <span className="line__inner-mask move-left--2">
              {SplitWord("a new", "export-line__inner")}
            </span>
            <span className="line__inner-mask">
              {SplitWord("Project?", "export-line__inner")}
            </span>
          </h3>
          <h4 className="contact-column__sub-title">
            <span className="line__inner-mask">
              {SplitWord("Or just say hello.", "export-line__inner")}
            </span>
          </h4>
        </div>
        <div className="contact-column contact-column__right">
          <div className="email-surrounding">
            <span className="email-section  line__inner-mask">
              <a
                href={`mailto:${email}`}
                className="contact-email link export-line__inner"
              >
                {email}
              </a>
            </span>
          ...
          <span className="line__inner-mask contact-column__social-media">
            {footer_social.map((social: SocialProp, i: number) => {
              return (
                social && (
                  <a
                    href={social.url}
                    key={i}
                    className="export-line__inner link"
                  >
          ...
          <div className="buy-me-cofee">
            <span className="line__inner-mask">
              <a
                className="bmc-button export-line__inner"
                target="_blank"
                rel="nofollow noopener noreferrer"
                href="https://www.buymeacoffee.com/nnajivictor"
              >
                <Coffee />
                <span
      ...
      <ContactFooter>
        <footer className="footer" id="slide-7">
          <CustomLink page="/about" className="footer__link">
            nnaji Victor
          </CustomLink>
          <a
            ...
            onMouseOver={handleFooterOver}
            onMouseOut={handleFooterOut}
          >
